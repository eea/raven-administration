...
from endpoints.processing.samplingpointgroups.models import (
    CreateGroupModel, DeleteGroupModel, AddMemberModel, RemoveMemberModel
)
...
def create_group():
    with CursorFromPool() as cursor:
        ...
        if not sp_ids:
            raise BadRequest("At least one sampling point is required")
        ...
        for sp_id in sp_ids:
            ...
            cursor.execute(
                "SELECT 1 FROM sampling_point_groups WHERE sampling_point_id = %(id)s",
                {"id": sp_id}
            )
            if cursor.fetchone():
...
def delete_group():
    with CursorFromPool() as cursor:
        ...
        cursor.execute("DELETE FROM sampling_point_groups WHERE group_id = %(id)s", model)
...
def add_member(group_id):
    with CursorFromPool() as cursor:
        ...
        cursor.execute(
            "SELECT sp.station_id FROM sampling_point_groups g "
            "JOIN sampling_points sp ON sp.id = g.sampling_point_id "
            "WHERE g.group_id = %(group_id)s LIMIT 1",
            {"group_id": group_id}
        )
        ...
        cursor.execute(
            "INSERT INTO sampling_point_groups (group_id, sampling_point_id) VALUES (%(group_id)s, %(id)s)",
...
def remove_member(group_id):
    with CursorFromPool() as cursor:
        ...
        cursor.execute(
            "DELETE FROM sampling_point_groups WHERE sampling_point_id = %(id)s AND group_id = %(group_id)s",
...
def lookup_samplingpoints():
    ...
    with CursorFromPool() as cursor:
        ...
        if group_id is not None:
            member_filter = "g.sampling_point_id IS NULL OR g.group_id = %(group_id)s"
            params["group_id"] = group_id
        else:
            member_filter = "g.sampling_point_id IS NULL"
        cursor.execute(f"""
            WITH
            {network_sql}
            SELECT
