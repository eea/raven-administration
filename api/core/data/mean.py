...
class Mean:
    ...
    def GetTimeseries(ids: tuple, cursor: any):
        sql = """
            ...
            and spo.pollutant = po.uri
            and spo.timestep = ti.id
            and spo.concentration = con.id
            and spo.id in %(ids)s
        """
    ...
    def Raw():
        sql = """
            SELECT
                ...
                CASE
                    WHEN o.verification_flag <= %(verificationFlag)s AND (o.validation_flag in (1,2,3,4) or %(useInvalidValues)s) AND (o.import_value != -9900)
    ...
    def Original():
        sql = """
            SELECT
                to_char (o.to_time, 'YYYY-MM-DD HH24:MI:SS') as "datetime", 
                CASE
                    WHEN o.verification_flag <= %(verificationFlag)s AND (o.validation_flag in (1,2,3,4) or %(useInvalidValues)s) AND (o.value != -9900)
        ...
        sql = """
            ...
            (
                SELECT 
                    ...
                    CASE 
                        WHEN o.verification_flag <= %(verificationFlag)s AND (o.validation_flag in (1,2,3,4) or %(useInvalidValues)s) AND (o.value != -9900) THEN  o.value
                        ELSE NULL 
                    END val
                FROM
                    observations o 
