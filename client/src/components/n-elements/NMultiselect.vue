...
const setSelectedOnOptions = () => {
    for (let i = 0; i < options.value.children.length; i++) {
        ...
        var values = p.modelValue.map(p => p.value);
        if (values.includes(v)) options.value.children[i].setAttribute("selected", true)
...
<template>
    <div class="n-elements n-multiselect" tabindex="0" @focusout="onFocusOut" :n-id="id">

        <div class="select" v-bind="$attrs" @click="expand = !expand">
            <div class="selections">
                <div v-if="!modelValue.length > 0">&nbsp;</div>
                <div class="selected-box" v-for="o in modelValue" @click.stop.prevent="onSelectionClick(o, $event)"> {{ o.label }} </div>
            </div>
