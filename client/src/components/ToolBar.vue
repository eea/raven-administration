...
<template>
  <div>
    ...
    <contextmenu :evt="ev" :show="showColumnPicker" @click-outside="showColumnPicker = false">
      <div v-for="column in columnPicker" class="pl-2 pr-4 py-1 flex cursor-pointer hover:bg-gray-100">
        <n-checkbox :modelValue="column.checked" v-on:update:model-value="columnsChanged(column)" :label="column.col"></n-checkbox>
      </div>
    </contextmenu>
  </div>
</template>
