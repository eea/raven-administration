...
const cmp_properties = computed(() => {
  ...
  if (!props.isEdit) return props.options.properties;
  return props.options.properties.filter((p) => p.showInEdit);
});

const cmp_required_properties = computed(() => {
  if (!cmp_properties.value) return [];
  return cmp_properties.value.filter((p) => !!p.required);
});
