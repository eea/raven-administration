    ...
    <container>
      <div class="flex gap-2">
        <div>
          ...
          <select class="select w-48 px-3 py-2" v-model="selectedYears" multiple size="5">
        ...
        <div>
          ...
          <select class="select w-48" v-model="selectedPollutant">
            <option v-for="pollutant in availablePollutants" :key="pollutant" :value="pollutant">{{ pollutant }}</option>
          </select>
        </div>
        <div>
          <div class="font-bold">Aggregation Process</div>
          <select class="select w-48" v-model="selectedAggregationProcess">
            <option v-for="process in availableAggregationProcesses" :key="process" :value="process">{{ process }}</option>
          </select>
        </div>
        <div>
          <div>&nbsp;</div>
          <button class="button" @click="showValues">Show statistics</button>
        </div>
        <div v-if="data.length > 0">
          <div>&nbsp;</div>
          <button class="button" @click="exportToCSV">Download CSV</button>
        </div>
      </div>
